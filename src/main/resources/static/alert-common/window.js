var win = new function () {
    ...
    // 确认框
    this.confirm = function (title, message, url) {
        this._close = function (r) {
            this.close();
        };
        var html = '<div class="win"><div class="mask-layer">' +
            '</div><div class="window-panel">' +
            '<iframe class="title-panel" frameborder="0" marginheight="0" marginwidth="0" scrolling="no"></iframe>' +
            '<div class="title"><h3></h3></div>' +
            '<a href="javascript:void(0)" onclick="javascript :history.back(-1)" class="close-btn" title="关闭">×</a><div class="body-panel"><p class="content"></p><p class="btns"><button class="w-btn" tabindex="1" onclick="javascript :history.back(-1);">确定</button><button class="w-btn" onclick="javascript :history.back(-1);">取消</button></p></div></div></div>';
